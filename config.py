...
##                               nickname: Fakeymacs
##
## Windows の操作を emacs のキーバインドで行うための設定（Keyhac版）ver.20190708_01
##
...
def configure(keymap):
    ...
    is_japanese_keyboard = False  # True
    ...
    # window_movement_key = [["A-S-b", "A-S-f"], ["A-S-Left", "A-S-Right"],
    #                        [None, "A-S-o"]]  # Multi-display
    ...
    desktop_switching_key = [  # ["A-C-b", "A-C-f"],
        ["W-C-Left", "W-C-Right"]
    ]  # for Windows 10

    # 仮想デスクトップを追加・削除するキーの組み合わせ（追加、削除 の順）を指定する（複数指定可）
    # custom.
    desktop_revising_key = [["W-C-Plus", "W-C-Minus"]]

    # タスクビューの表示を切り替えるキーを指定する
    # custom.
    toggle_taskview_key = ["W-C-Up", "W-C-Down"]
    ...
    keymap_global = keymap.defineWindowKeymap()  # custom.
    ...
    if use_emacs_ime_mode:
        keymap_emacs = keymap.defineWindowKeymap(
            check_func=
            lambda wnd: is_emacs_target(wnd) and not is_emacs_ime_mode(wnd))
        keymap_ime = keymap.defineWindowKeymap(
            check_func=
            lambda wnd: is_ime_target(wnd) and not is_emacs_ime_mode(wnd))
    else:
    ...
    def toggle_input_method():
        ...
        if not fakeymacs.is_playing_kmacro:
            ...
            keymap.popBalloon("ime_status", message, 1500)  # 500
    ...
    def kmacro_end_macro():
        ...
        if ctl_x_prefix_key and len(keymap.record_seq) >= 4:
            if (((keymap.record_seq[len(keymap.record_seq) - 1] ==
                  (ctl_x_prefix_vkey[0], True) and
                  keymap.record_seq[len(keymap.record_seq) - 2] ==
                  (ctl_x_prefix_vkey[1], True)) or
                 (keymap.record_seq[len(keymap.record_seq) - 1] ==
                  (ctl_x_prefix_vkey[1], True) and
                  keymap.record_seq[len(keymap.record_seq) - 2] ==
                  (ctl_x_prefix_vkey[0], True))) and
                    keymap.record_seq[len(keymap.record_seq) -
    ...
    def checkWindow(processName, className, window=None):
        ...
        return ((processName is None or
                 fnmatch.fnmatch(window.getProcessName(), processName)) and
                (className is None or
                 fnmatch.fnmatch(window.getClassName(), className)))
    ...
    def digit(number):
        def _func():
            if fakeymacs.is_universal_argument:
                ...
                reset_undo(
                    reset_counter(
                        reset_mark(repeat(self_insert_command2(str(
                            number))))))()
    ...
    def reset_region():
        if use_region_reset and fakeymacs.is_marked and fakeymacs.forward_direction is not None:
            ...
            if (checkWindow("sakura.exe", "EditorClient") or  # Sakura Editor
                    checkWindow("sakura.exe", "SakuraView166")
                    or  # Sakura Editor
                    checkWindow("Code.exe",
                                "Chrome_WidgetWin_1") or  # Visual Studio Code
                    checkWindow("Hidemaru.exe",
    ...
    for vkey in range(VK_A, VK_Z + 1):
        s_vkey = "(" + str(vkey) + ")"
        define_key(keymap_emacs, s_vkey,
                   reset_undo(
                       reset_counter(
    ...
    # custom.
    define_key(
        keymap_emacs, "M-S-Left",
        reset_search(
            reset_undo(
                reset_counter(mark(repeat(select_backward_word), False)))))
    # custom.
    define_key(
        keymap_emacs, "M-S-Right",
        reset_search(
            reset_undo(reset_counter(mark(repeat(select_forward_word),
                                          True)))))
    define_key(
        keymap_emacs, "Up",
        reset_search(
    ...
    define_key(
        ...
        reset_search(
            reset_undo(reset_counter(reset_mark(repeat3(
                backward_kill_word))))))
    define_key(keymap_emacs, "M-d",
               reset_search(
                   reset_undo(reset_counter(reset_mark(repeat3(kill_word))))))
    define_key(keymap_emacs, "C-k",
               reset_search(
                   reset_undo(reset_counter(reset_mark(repeat3(kill_line))))))
    define_key(
        keymap_emacs, "C-w",
    ...
    define_key(keymap_emacs, "C-c",
               reset_search(reset_undo(reset_counter(reset_mark(copy)))))
    define_key(
        keymap_emacs, "C-v",
        reset_search(reset_undo(reset_counter(reset_mark(repeat(
            yank))))))  # scroll_key の設定で上書きされない場合
    define_key(keymap_emacs, "C-z",
               reset_search(reset_counter(reset_mark(undo))))

    # C-Underscore を機能させるための設定
    if is_japanese_keyboard:
        define_key(keymap_emacs, "C-S-BackSlash",
                   reset_search(reset_undo(reset_counter(reset_mark(undo)))))
    else:
        define_key(keymap_emacs, "C-S-Minus",
                   reset_search(reset_undo(reset_counter(reset_mark(undo)))))

    if is_japanese_keyboard:
        # C-Atmark だとうまく動かない方が居るようなので C-(192) としている
        # （http://bhby39.blogspot.jp/2015/02/windows-emacs.html）
        define_key(keymap_emacs, "C-(192)",
                   reset_search(reset_undo(reset_counter(set_mark_command))))
    else:
        # C-S-2 は有効とならないが、一応設定は行っておく（C-S-3 などは有効となる。なぜだろう？）
        define_key(keymap_emacs, "C-S-2",
                   reset_search(reset_undo(reset_counter(set_mark_command))))

    define_key(keymap_emacs, "C-Space",
               reset_search(reset_undo(reset_counter(set_mark_command))))
    define_key(keymap_emacs, "Ctl-x h",
               reset_search(reset_undo(reset_counter(mark_whole_buffer))))
    define_key(keymap_emacs, "Ctl-x C-p",
    ...
    define_key(
        keymap_emacs, "W-v",
        reset_search(reset_undo(reset_counter(reset_mark(repeat(yank))))))
    define_key(keymap_emacs, "W-z",
               reset_search(reset_counter(reset_mark(undo))))
    define_key(keymap_emacs, "W-a",
               reset_search(reset_undo(reset_counter(mark_whole_buffer))))
    define_key(
        keymap_emacs, "W-p",
        reset_search(reset_undo(reset_counter(reset_mark(print_buffer)))))
    define_key(keymap_emacs, "W-n",
               reset_search(reset_undo(reset_counter(reset_mark(new_window)))))
    define_key(keymap_emacs, "W-t",
               reset_search(reset_undo(reset_counter(reset_mark(new_tab)))))
    define_key(keymap_emacs, "W-f",
               reset_undo(reset_counter(reset_mark(isearch_forward))))
    define_key(keymap_emacs, "W-b",
               reset_search(reset_undo(reset_counter(reset_mark(bold_font)))))
    define_key(
        keymap_emacs, "W-i",
        reset_search(reset_undo(reset_counter(reset_mark(italic_font)))))
    define_key(
        keymap_emacs, "W-u",
        reset_search(reset_undo(reset_counter(reset_mark(underline_font)))))
    define_key(
        keymap_global, "W-C-f12",
        reset_search(reset_undo(reset_counter(reset_mark(action_center)))))
    define_key(
        keymap_global, "W-C-f11",
        reset_search(reset_undo(reset_counter(reset_mark(show_desktop)))))
    define_key(keymap_global, "W-q",
               reset_search(reset_undo(reset_counter(reset_mark(quit_app)))))
    define_key(
        keymap_emacs, "W-w",
        reset_search(reset_undo(reset_counter(reset_mark(kill_buffer)))))
    ...
    if use_emacs_ime_mode:
        ...
        def ei_popBalloon(toggle):
            if not fakeymacs.is_playing_kmacro:
                if emacs_ime_mode_balloon_message:
                    if fakeymacs.ei_last_window:
                        keymap.popBalloon("emacs_ime_mode",
                                          emacs_ime_mode_balloon_message,
                                          1500)  # custom.
                    else:
                    ...
                    if toggle:
                        if fakeymacs.ei_last_window:
                            ...
                            message = "[main]"
                        keymap.popBalloon("emacs_ime_mode", message,
                                          1500)  # 500
        ...
        for vkey in vkeys():
            ...
            define_key(keymap_ei, "C-" + s_vkey,
                       ei_record_func(self_insert_command("C-" + s_vkey)))
            define_key(keymap_ei, "C-S-" + s_vkey,
                       ei_record_func(self_insert_command("C-S-" + s_vkey)))
            define_key(keymap_ei, "A-" + s_vkey,
                       ei_record_func(self_insert_command("A-" + s_vkey)))
            define_key(keymap_ei, "A-S-" + s_vkey,
                       ei_record_func(self_insert_command("A-S-" + s_vkey)))
        ...
        ## 「スクロール」のキー設定（上書きされないように最後に設定する）
        if scroll_key:
            define_key(keymap_ei, scroll_key[0] and
                       scroll_key[0].replace("M-", "A-"),
                       ei_record_func(scroll_up))
            define_key(keymap_ei, scroll_key[1] and
                       scroll_key[1].replace("M-", "A-"),
            ...
            if wnd.isVisible() and not wnd.getOwner():
                ...
                if class_name == "Emacs" or title != "":
                    ...
                    if not re.match(r"Progman$", class_name):
                        ...
                        if not re.match(
                                r"RocketDock.exe$",
                                process_name):  # サンプルとして RocketDock.exe を登録
    ...
    if window_switching_key:
        for previous_key, next_key in window_switching_key:
            ...
            define_key(keymap_global, next_key,
                       reset_search(
    ...
    if window_movement_key:
        for previous_key, next_key in window_movement_key:
            define_key(
                keymap_global, previous_key,
                reset_search(
                    reset_undo(reset_counter(reset_mark(previous_display)))))
            define_key(
                keymap_global, next_key,
                reset_search(
                    reset_undo(reset_counter(reset_mark(next_display)))))
    ...
    # 仮想デスクトップの追加・削除
    # custom.
    if desktop_revising_key:
        for append_key, delete_key in desktop_revising_key:
            define_key(
                keymap_global, append_key,
                reset_search(
                    reset_undo(reset_counter(reset_mark(append_desktop)))))
            define_key(
                keymap_global, delete_key,
                reset_search(
                    reset_undo(reset_counter(reset_mark(delete_desktop)))))

    # タスクビューの表示の切り替え
    # custom.
    if toggle_taskview_key:
        for key in toggle_taskview_key:
            define_key(
                keymap_global, key,
                reset_search(
                    reset_undo(reset_counter(reset_mark(toggle_taskview)))))

    # IME の「単語登録」プログラムの起動
    define_key(keymap_global, word_register_key,
    ...
    if scroll_key:
        define_key(keymap_lw, scroll_key[0] and
                   scroll_key[0].replace("M-", "A-"), scroll_up)
        define_key(keymap_lw, scroll_key[1] and
                   scroll_key[1].replace("M-", "A-"), scroll_down)
    ...
    define_key(keymap_lw, "S-Enter",
               lw_exit_search(self_insert_command("S-Enter")))
    define_key(keymap_lw, "C-Enter",
               lw_exit_search(self_insert_command("C-Enter")))
    define_key(keymap_lw, "A-Enter",
               lw_exit_search(self_insert_command("C-Enter")))
    ...
    if 0:  # 1
        ...
        def lw_lancherList():
            def popLancherList():
                ...
                window_items = []
                if window_list:
                    processName_length = max(
                        map(len, map(Window.getProcessName, window_list)))
                ...
                application_items = [
                    ...
                    [
                        "sakura",
                        keymap.ShellExecuteCommand(
                            None, r"C:\Program Files (x86)\sakura\sakura.exe",
                            "", "")
                    ],
                    [
                        "explorer",
                ...
                website_items = [
                    ...
                    [
                        "NTEmacs＠ウィキ",
                        keymap.ShellExecuteCommand(
                            None, r"http://www49.atwiki.jp/ntemacs/", "", "")
                    ],
                ]
    ...
    if 0:
        ...
        keymap_real_emacs["(242)"] = keymap.InputKeyCommand(
            "C-Yen")  # [カタカナ・ひらがな] キー
        keymap_real_emacs["(243)"] = keymap.InputKeyCommand(
            "C-Yen")  # [半角／全角] キー
        keymap_real_emacs["(244)"] = keymap.InputKeyCommand(
            "C-Yen")  # [半角／全角] キー
